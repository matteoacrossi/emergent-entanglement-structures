...
def CL(G):
    ...
    sums = np.sum(mx2) - np.trace(mx2) # calculate sum over all elements except diagonal ones
...
def CV_i(G, filter = 1.):
    ...
    a_ij = 1. - np.identity(mx.shape[0]) # create an "inverse" identity matrix
    ...
    shp = mx.shape[0]
    mmx = np.broadcast_to(mx, (shp,shp,shp,)) # broadcast matrix to accommodate for third index
...
def D_g(G, binarized = False):
    ...
    M = nx.attr_matrix(G, edge_attr='weight')[0] # get weight matrix
    if binarized:
        M = np.logical_not(np.isclose(M,0.))
    strengths = np.sum(M, axis = 0) # strengths
...
def P_ij(G, i, j):
    L = G.number_of_nodes()
    weights = np.array(nx.attr_matrix(G, edge_attr='weight')[0]) # get matrix of weights
...
def draw_entanglement_graph(G,
                            layout="circular",
                            edge_scale_factor=1.,
                            node_scale_factor=1.,
                            labels={}, cmap=plt.cm.jet,
                            node_color="#0A7290",
                            edge_offset=0.,
                            **kwargs):
    ...
    if layout == "circular":
        pos = {node: np.array([np.cos(2. * np.pi * float(i) / float(nq) + th),\
                            np.sin(2. * np.pi * float(i) / float(nq) + th)]) for i, node in enumerate(G.nodes)}
        label_pos = [pos[i]*1.15 for i in pos]
    if layout == "spring":
        pos = nx.spring_layout(G)

    #minv, maxv = (0.007174451964390649, 0.1706297630123893)
    minv, maxv = (0.0, .75)#0.75)

    edgewidth = [edge_offset+d['weight']*10*edge_scale_factor for (u,v,d) in G.edges(data=True)]
    edgecolors = [cmap((d['weight'] - minv) / (maxv - minv) + edge_offset) for (u,v,d) in G.edges(data=True)]
    #print(edgecolors)
    nodesize = [(e[1]*200+10)*node_scale_factor for e in G.degree(weight='weight')]
    ...
    nx.draw_networkx_nodes(G, pos, node_size=nodesize, node_color=node_color, edgecolors="k", linewidths = 0.3, cmap=cmap, **kwargs)
